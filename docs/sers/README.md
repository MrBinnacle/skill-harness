...
| `arms` | Which arms ran: the run's declared arm names. `null` and `full` are the per-clause instrument's two-value vocabulary; a declared-arm run names its own arms (a factorial's cells, for example). One arm is a string; several is an array of unique names. |
