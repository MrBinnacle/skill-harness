...
### [`docs/assurance/halfupdate-tie-migration-mutation-receipt.md`](assurance/halfupdate-tie-migration-mutation-receipt.md)

The mutation receipt for the #368 Path C migration (Gate-2 discordant
stopping for tie-heavy clauses), generated by
`scripts/mutation_receipt.py` into
[`halfupdate-tie-migration-mutation-receipt.json`](assurance/halfupdate-tie-migration-mutation-receipt.json).

- **Claims:** Three named mutants of `src/skill_harness/ablation/gate2_stopping.py`
  were each run in its own git worktree at commit `36bf46a` under Python 3.13.15
  against the file at `sha256:b6b9e5c04b705ebf0e02cab01f42cd1391333d23d0d5af184f3153044cf3c99a`,
  and all three were KILLED by named test nodes: removing the scalar fallback
  so Gate-2 UNRESOLVED always returns inconclusive (M-G1), swapping the
  pass/fail thresholds so high-probability scenarios fail the wrong condition
  (M-G2), and zeroing the posterior parameters so they no longer match the
  drop-ties recompute (M-G3). Each case asserted a clean baseline passing
  first with nonzero collection, differing source digests, a mutant that
  imports, and a byte-unchanged production tree. The production path is the
  ablation runner's `DiscordantStoppingAccumulator`; the receipt attests the
  decision rule, not the runner import (that seam is pinned by the sensitivity
  suite's runner tests).
- **Refuses to claim:** A mutation score — three hand-chosen mutants cannot
  support one; adequacy of the `test_halfupdate_tie_sensitivity.py` suite as a
  whole; that the Gate-2 decision rule (gamma, delta_min, q_min) is tested here
  (tested by `tests/test_oc_gate2.py`); that the zero-tie scalar path is tested
  here (tested by the existing `stopping.py` seam tests).
