...
### [`docs/assurance/anchor-pin-mutation-receipt.md`](assurance/anchor-pin-mutation-receipt.md)

The mutation receipt for the #557 pre-flight dependency-anchor check
(`scripts/check_dependency_anchor.py`), which refuses a dependency bump an
exact-pinned anchor cannot satisfy before the test matrix runs. Generated
by `scripts/mutation_receipt.py --select 557-anchor-pin` into
...
- **Claims:** One named mutant (M-A1) was run in its own git worktree at
  ...
  exact-pin comparison never returns a blocking `==` pin, which is the shape
  that lets a follower bump above an `==` anchor read as a pass — the #549
  failure mode. The case asserted its clean baseline passed first with
  nonzero collection (two tests: the #549 kill fixture and a coordinated-bump
  control), that the imported `module.__file__` resolved inside its own
  worktree, that the source digests differed, that the mutant imported, and
  that the production tree was byte-unchanged afterwards. The control
  (`TestPassesCoordinatedBump::test_passes`) stayed green under the mutant,
  excluding an empty-cell kill: the mutant flipped the refusal path and left
  the pass path intact.
