...
- `run evaluate-skill` report: `"2.0.0"`. Initial `"1.0.0"`. Bumped to `"1.1.0"` for A55 comparability axes (`subject_model`, `user_message_sha256`). Bumped to `"1.2.0"` for the `coverage_warnings` field on `vector` (M3 pre-tag fix). Bumped to `"1.3.0"` for `is_prior_only` on `ClauseReport` (B5 hostile-review fix). Bumped to `"1.4.0"` for the #187 anytime-valid confidence-sequence fields. Bumped to `"2.0.0"` at the merge of `agent/issue-360`, because the `aggregation_method` enum value `bh_fdr_fallback` was renamed to `bounded_pooling_refused`. This line read `"1.2.0"` until that merge, two bumps stale. `REPORT_SCHEMA_VERSION` in `src/skill_harness/aggregation/report.py` had been `"1.4.0"` since #187.
...
**The owed wire-format version bump is TAKEN at this merge.** `report_schema_version` moves from `"1.4.0"` to `"2.0.0"`. The forcing change is a rename. This branch replaced the `aggregation_method` enum value `bh_fdr_fallback` with `bounded_pooling_refused`, and the rule stated above classifies removals, renames and type changes as a major bump because they break `diff skill` consumers. The bump therefore ends the `1.x` additive-only run this section opens with. The deferral stays recorded here, because it was a deliberate decision. The bump was withheld while the branch stayed unmerged, since the branch waited on the confirmatory run of v2 section 5 and a bump would have made every development report on the branch incomparable with the ones already recorded against it. That run reported NOT_REJECTED on 2026-09-13, which is the point the deferral named. `#441`, the ticket the deferral was tracked on, closed on 2026-09-06, so the obligation outlived its tracker.
...
**`aggregation_provenance` required sub-keys (when `aggregation_method = ebmom_hierarchical`):** `alpha_hat, beta_hat, sample_mean, sample_var, K_clauses, pythonhashseed`, and `admitted_bootstrap = {mechanism, spec, s_target, block, max_blocks, seed, drawn, kept, below_crit, nonpositive_c, exhausted, fell_back_to_plugin, used}`.

**`admitted_bootstrap` was ADDED on 2026-09-05**, on the unmerged `agent/issue-360` branch only, under section 4 of `docs/assurance/ebmom-peel-preregistration-amendment-v2.md`, which replaced the plug-in decision on the admitted path with an admission-conditioned parametric bootstrap. It changes what `clauses[].p_win_gt_threshold` MEANS on that path: the number is the clause tail probability averaged over `used` draws of the hyperparameters, not `sf(0.60)` of the reported `posterior_alpha`/`posterior_beta`. Those two therefore disagree on an admitted fit, deliberately -- no single Beta carries a mixture's tail, and reporting a moment-matched stand-in would name a distribution the run never used. `drawn`, `kept`, `below_crit` and `nonpositive_c` are the draw budget's accounting; `exhausted` says the budget ran out before `s_target` draws were kept; `fell_back_to_plugin` says NO draw was admissible, in which case the tail IS the plug-in's and the decision is the plug-in's, counted rather than silent. `seed` derives from the canonical clause encoding under section 3 of the v1 amendment with the label `pb`, so an admitted decision is reproducible from the receipt.
